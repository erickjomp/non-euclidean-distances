...
list_breaks <- list(
  c(0, seq(400,1300,300), Inf),  # reg 2
...
#### output
file_save_fig_gen <- 
  "03_raster_interpolation//ras_yearly_avg_IED_IED-NED1_reg%s.pdf"
file_save_fig_article_gen <- "article//figs/fig_ras_yearly_avg_IED_IED-NED1_reg%s.pdf"


file_save_fig_regsjoined = "03_raster_interpolation//ras_yearly_avg_IED_IED-NED1_regs2-7.pdf"
file_save_fig_article_regsjoined = "article//figs/fig_ras_yearly_avg_IED_IED-NED1_regs2-7.pdf"

file_save_fig_topo_regs = "03_raster_interpolation/ras_topo_regs2-7.pdf"
file_save_fig_topo_article_regs = "article//figs/fig_ras_topo_regs2-7.pdf"

  
i_letter = 1
list_plots_topo = list()
...
list_plots <- 
  lapply(id_regs, function(id_reg){
    ...
    df_ras_differences = df_ras_methods %>% spread("method","pry") %>% 
      # mutate_at(4,~ .x - `IED`) %>% 
      mutate(diff = `IED - NED1` - IED,
             diff_perc = diff / IED * 100) %>% 
      select(x,y,diff,diff_perc) %>% 
      mutate(method = "IED-NED1",.before = 3)
      # gather(diff, pry,-c(1,2)) %>% 
    ...
    ( plot_ras_methods <-
        ...
        guides(fill = guide_colorsteps(#show.limits = T,
          ...
          barheight = 1, draw.llim = T, barwidth = 10
          # title.position = "top"
        )) +
    ...
    ( 
      plot_ras_diff <-
        df_ras_differences %>% mutate(method = "(mm)") %>% 
        ggplot() + 
        # geom_tile(aes(fill = z)) +
        # geom_contour_filled(aes(x = gdat$x, y = gdat$y,z = distance)
        #                     ,breaks = breaks,color = NA) + # bins = 18,
        geom_contour_fill(aes(x = df_ras_differences$x, y = df_ras_differences$y, z = diff,
                              fill = stat(level)),
        ...
        labs(x = NULL, y = NULL, #, fill = "Precipitation\ndifference\n(mm)") + 
             title = "Precipitation difference",
             fill = NULL) +  
        scale_x_continuous(expand = c(0, 0.018)) +
        # scale_y_continuous(expand = c(0, 0)) +
        theme_minimal(base_size = base_size_figs) +
    ...
    breaks_diff_perc = c(-Inf,seq(-20,20,5),Inf)
    ( 
      plot_ras_diff_perc <-
        df_ras_differences %>% mutate(method = "(%)") %>% 
        ggplot() + 
        # geom_tile(aes(fill = z)) +
        # geom_contour_filled(aes(x = gdat$x, y = gdat$y,z = distance)
        #                     ,breaks = breaks,color = NA) + # bins = 18,
        geom_contour_fill(aes(x = df_ras_differences$x, y = df_ras_differences$y, 
                              z = diff_perc,
                              fill = stat(level)),
                          breaks = breaks_diff_perc,
                          color = NA) +
        guides(fill = guide_colorsteps(#show.limits = T,
          # barheight = 10, draw.llim = T, barwidth = 0.8,
          barheight = 1, draw.llim = T, barwidth = 8
          # title.position = "top"
        )) +
        scale_fill_brewer(type = "div", palette = "Spectral") +
        # scale_fill_viridis_d(option = "viridis", direction = -1,) + 
    ...
    (
      plot_topo = 
        ...
        guides(fill = guide_colorsteps(#show.limits = T,
          barheight = 6, draw.llim = T, barwidth = 0.8,
        ...
        labs(x = NULL, y = NULL, title = sprintf("Region %s elevation (masl)",id_reg), fill = NULL) +
        ...
        theme(legend.position = "inside",
              legend.position.inside = c(0.0, 0.0),legend.justification = c(0,0),
              axis.text = element_blank(),
              axis.ticks = element_blank(),
    ...
    letters_sel = LETTERS[c(i_letter, i_letter+1)]
    i_letter <<- i_letter +2
    (
      plot_final = cowplot::plot_grid(#plot_topo,
                                      plot_ras_methods, plot_ras_diff,
                                      plot_ras_diff_perc, 
                                      nrow = 1,
                                      labels = c(letters_sel[1],letters_sel[2],NA),
                                      rel_widths = c(2,1,1))
    )
    ...
    list_plots_topo[[as.character(id_reg)]] <<- plot_topo
    list_plots_topo <<- list_plots_topo
    message("The average difference (mm) is: ", mean(abs(df_ras_differences$diff)))
    message("The average difference (%) is: ", mean(abs(df_ras_differences$diff_perc)))
    return(plot_final)
...
plot_final = cowplot::plot_grid(list_plots[[1]],list_plots[[2]],ncol = 1,
                                rel_heights = c(1.047619,1))

ggsave(plot_final,
       filename = file_save_fig_regsjoined ,
       width = 190,height = 215,units = "mm")

ggsave(plot_final,
       filename = file_save_fig_article_regsjoined ,
       width = 190,height = 215,units = "mm")


plot_topos = cowplot::plot_grid(list_plots_topo[[1]], list_plots_topo[[2]], nrow = 1)
ggsave(plot_topos,
       filename = file_save_fig_topo_regs,
       width = 90,height = 70,units = "mm")

ggsave(plot_topos,
       filename = file_save_fig_topo_article_regs ,
       width = 90,height = 70,units = "mm")
