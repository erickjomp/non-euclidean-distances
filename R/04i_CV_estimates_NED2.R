  ...
  for(id_reg in 1:12){
    ...
    list_out <- 
      NED_interpolation(formula = pr~elev, 
                        ...
                        list_mat_dists_loc = list_mat_dists_loc_in[1:50],
                        idp = 2,
    ...
    # return(df_sim[,c(1, which(ista) + 1)])
  } #%>% do.call(rbind,.)

list_df_sims_regs %>% 
  saveRDS(
    "04_cross_validation//estimates/list_df_m_inregs_est_NED2.rds")
list_best_lambdas_CV %>% 
  saveRDS(
    "04_cross_validation//list_best_lambdas_CV_NED2.rds")
