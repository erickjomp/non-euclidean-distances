...
file_save_raster_NED_gen <-
  "03_raster_interpolation//raster_inter/ras_NED1_reg%s.nc"
file_save_raster_avg_NED_gen <-
  "03_raster_interpolation//raster_avg_yearly/ras_NED1_reg%s.tif"
file_df_lambdas <- 
  "03_raster_interpolation/df_lambdas_NED1.rds"
file_list_df_error_lambdas <- 
  "03_raster_interpolation/list_df_error_lambdas_NED1.rds"


df_lambdas_NED1 <- data.frame()
list_df_error_lambdas_NED1 <- list()
...
for (id_reg in 1:12) {
  ...
  df_lambdas_NED1 <- 
    rbind(df_lambdas_NED1, 
          data.frame(id_reg = id_reg, lambda = list_out$lambda))
  
  list_df_error_lambdas_NED1[[id_reg]] <- 
    list_out$WS
}

saveRDS(df_lambdas_NED1, file_df_lambdas)
saveRDS(list_df_error_lambdas_NED1,
        file_list_df_error_lambdas)
